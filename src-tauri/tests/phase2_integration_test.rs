...
async fn test_phase2_semantic_analysis_integration() {
    ...
    assert!(
        !analysis_result.keyword_matches.is_empty(),
        "Should find keyword matches"
    );
    assert!(
        analysis_result.semantic_similarity_score >= 0.0,
        "Semantic similarity should be valid"
    );
    assert!(
        analysis_result.industry_relevance_score >= 0.0,
        "Industry relevance should be valid"
    );
    assert!(
...
async fn test_phase2_industry_analysis_integration() {
    ...
    // Test data
    let resume_content =
        "Senior software engineer with 7 years experience leading teams and architecting solutions";
    let job_description = "Looking for a senior engineer with leadership experience";
    let industry = "technology";

    // Perform industry analysis
    let result = analyzer
        .analyze_for_industry(resume_content, job_description, industry)
        .await;
    ...
    // Verify results structure
    assert!(
        !analysis_result.detected_industry.is_empty(),
        "Should detect an industry"
    );
    assert!(
        analysis_result.confidence_score >= 0.0,
        "Confidence score should be valid"
...
async fn test_phase2_enhanced_scoring_integration() {
    ...
    // Verify results structure
    assert!(
        analysis_result.base_analysis.overall_score >= 0.0,
        "Overall score should be valid"
    );
    assert!(
        !analysis_result.semantic_analysis.keyword_matches.is_empty(),
    ...
    assert!(
        analysis_result
            ...
            >= 0.0,
        "Should have ATS compatibility"
    );
    assert!(
        !analysis_result.scoring_breakdown.weighted_scores.is_empty(),
...
async fn test_phase2_prompt_engineering_integration() {
    ...
    // Verify results structure
    assert!(
        !prompt_response.formatted_prompt.is_empty(),
        "Should have formatted prompt"
    );
    assert!(
        !prompt_response.model_config.model_name.is_empty(),
        "Should have model config"
    );
    assert!(
        prompt_response.estimated_tokens > 0,
        "Should estimate tokens"
    );
    assert!(
        !prompt_response.prompt_strategy.is_empty(),
...
async fn test_phase2_ats_simulation_integration() {
    ...
    let result = simulator
        .simulate_ats_processing(resume_content, &target_keywords)
        .await;
    ...
    // Verify results structure
    assert!(
        simulation_result.overall_ats_score >= 0.0,
        "Overall ATS score should be valid"
    );
    assert!(
        !simulation_result.system_simulations.is_empty(),
        "Should have system simulations"
    );
    assert!(
        simulation_result
            .parsing_analysis
    ...
    assert!(
        simulation_result.keyword_extraction.keywords_found.len() >= 2,
        "Should find some keywords"
    );
    assert!(
        !simulation_result.optimization_recommendations.is_empty(),
    ...
    println!(
        "   🎯 Overall ATS Score: {:.1}%",
        simulation_result.overall_ats_score * 100.0
    );
    println!(
        "   📞 Contact extraction confidence: {:.1}%",
        simulation_result
            .parsing_analysis
            .contact_info_extraction
...
async fn test_phase2_complete_workflow_integration() {
    ...
    let ats_result = ats_simulator
        .simulate_ats_processing(resume_content, &target_keywords)
        .await
        .expect("ATS simulation should succeed");
    ...
    let prompt_result = prompt_engine
        .create_enhanced_prompt(prompt_request)
    ...
    assert!(
        semantic_result.confidence_score > 0.5,
        "Should have good semantic confidence"
    );
    assert!(
        !semantic_result.keyword_matches.is_empty(),
        "Should match keywords"
    );

    // Industry analysis verification
    assert_eq!(
        industry_result.detected_industry, "technology",
        "Should detect technology industry"
    );
    assert_eq!(
        industry_result.role_level_assessment.detected_level, "senior",
        "Should detect senior level"
    );

    // Enhanced scoring verification
    assert!(
        comprehensive_result.base_analysis.overall_score > 60.0,
    ...
    assert!(
        ats_result.overall_ats_score > 0.5,
        "Should have good ATS compatibility"
    );
    assert!(
        !ats_result.system_simulations.is_empty(),
        "Should simulate multiple ATS systems"
    );
    assert!(
        ats_result
            .parsing_analysis
            .contact_info_extraction
            .email_detected,
        "Should detect contact info"
    );

    // Prompt engineering verification
    assert!(
        prompt_result.formatted_prompt.contains("Python"),
        "Prompt should contain relevant skills"
    );
    assert!(
        prompt_result.estimated_tokens > 100,
    ...
    println!(
        "   📈 Overall Score: {:.1}",
        comprehensive_result.base_analysis.overall_score
    );
    println!(
        "   🎯 Semantic Confidence: {:.1}%",
        semantic_result.confidence_score * 100.0
    );
    println!(
    ...
    println!(
        "   🏢 ATS Compatibility: {:.1}%",
        ats_result.overall_ats_score * 100.0
    );
    println!(
        "   🔍 Keywords Found: {}/{}",
