...
test.describe('Resume Optimization Workflow', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="optimization-page"]')).toBeVisible();
  });

  test('should load optimization workspace', async ({ page }) => {
    // Check that all main components are present
    await expect(page.locator('[data-testid="original-resume-editor"]')).toBeVisible();
    await expect(page.locator('[data-testid="optimized-resume-editor"]')).toBeVisible();
    await expect(page.locator('[data-testid="optimization-controls"]')).toBeVisible();
  });
...
- Python
    ...
    const originalEditor = page.locator('[data-testid="original-resume-editor"]');
...
- Strong communication skills
  ...
  test('should perform optimization with different levels', async ({ page }) => {
    ...
    await page.fill('[data-testid="original-resume-editor"]', 'Original resume content');
    await page.fill('[data-testid="job-description-input"]', 'Job requirements');

    // Test Conservative optimization
    await page.selectOption('[data-testid="optimization-level"]', 'Conservative');
    await page.click('[data-testid="optimize-button"]');

    // Wait for optimization to complete
    await expect(page.locator('[data-testid="optimization-loading"]')).toBeVisible();
    await expect(page.locator('[data-testid="optimized-content"]')).toBeVisible({ timeout: 15000 });

    // Verify results
    await expect(page.locator('[data-testid="before-score"]')).toContainText('75');
    await expect(page.locator('[data-testid="after-score"]')).toContainText('88');
    await expect(page.locator('[data-testid="improvement-percentage"]')).toContainText('17.3%');
  });
  ...
  test('should show real-time analysis when enabled', async ({ page }) => {
    ...
    await page.route('**/analyze_resume', route => {
      route.fulfill({
        ...
        body: JSON.stringify({
          ...
          data: {
            overall_score: 82,
            category_scores: { skills: 85, experience: 78, education: 80, keywords: 85, format: 90 },
    ...
    await page.fill('[data-testid="original-resume-editor"]', 'John Doe\nSoftware Engineer');
    await page.fill('[data-testid="job-description-input"]', 'Looking for a developer');

    // Wait for real-time analysis to trigger
    await expect(page.locator('[data-testid="live-score"]')).toBeVisible({ timeout: 5000 });
    await expect(page.locator('[data-testid="live-score"]')).toContainText('82');
  });
  ...
  test('should display change tracking', async ({ page }) => {
          ...
          data: {
            ...
            changes_made: [
              ...
              },
              {
                ...
                optimized: 'Architected and developed scalable web applications',
                impact_score: 0.7
              }
            ],
            before_score: 70,
    ...
    await expect(page.locator('[data-testid="changes-tracking"]')).toBeVisible({ timeout: 15000 });
    
    // Check that changes are displayed
    await expect(page.locator('[data-testid="change-item"]')).toHaveCount(2);
    await expect(page.locator('[data-testid="change-impact"]').first()).toContainText('0.9');
  });
  ...
  test('should allow exporting optimized resume', async ({ page }) => {
    ...
    expect(download.suggestedFilename()).toMatch(/optimized.*resume.*\.(txt|docx|pdf)$/);
  });
  ...
  test('should preserve user input during optimization', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="original-resume-editor"]')).toHaveValue(originalContent);
    await expect(page.locator('[data-testid="job-description-input"]')).toHaveValue(jobContent);
