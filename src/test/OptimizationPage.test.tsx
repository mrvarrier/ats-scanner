...
describe('OptimizationPage', () => {
  ...
  it('performs optimization and displays results', async () => {
    ...
    await waitFor(() => {
      expect(mockInvoke).toHaveBeenCalledWith('optimize_resume', expect.objectContaining({
    ...
    await waitFor(() => {
      const optimizedTextarea = screen.getByDisplayValue(/5\+ years Python development/i)
  ...
  it('allows copying optimized content', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByDisplayValue(/5\+ years Python development/i)).toBeInTheDocument()
    })
