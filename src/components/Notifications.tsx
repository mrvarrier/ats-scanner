...
function NotificationItem({ notification }: { notification: Notification }) {
  ...
  return (
    <div
      className={`rounded-lg border-l-4 bg-white shadow-lg ${getBorderColor()} ${getBgColor()} transform p-4 transition-all duration-300 ease-in-out animate-in slide-in-from-right-full`}
    >
  ...
  const handleError = useCallback(
    (err: Error | unknown, context?: string) => {
      ...
      if (message.toLowerCase().includes('network')) {
        ...
        message =
          'The AI service is not available. Please ensure Ollama is running';
      }
