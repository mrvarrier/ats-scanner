...
export function VirtualizedTable<T extends Record<string, any>>({
  ...
  }, [processedData, columns, selectable, isSelected, toggleSelection, onRowClick, onRowDoubleClick, rowClassName])

  // Header component
  const Header = () => (
    <div className={`bg-gray-50 border-b ${stickyHeader ? 'sticky top-0 z-10' : ''} ${headerClassName}`}>
      {/* Global filter */}
      {filterable && (
      ...
      <div className="flex items-center">
        ...
        {columns.map((column) => (
          <div
            key={String(column.key)}
            className="px-4 py-3 font-medium text-left cursor-pointer hover:bg-gray-100 flex items-center gap-2"
            style={{ 
              width: column.width || 150,
              minWidth: column.minWidth,
              maxWidth: column.maxWidth
